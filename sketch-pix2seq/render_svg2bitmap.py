...
def main(**kwargs):
    ...
    for dataset_i in range(len(model_params.data_set)):
        ...
        # Load only the current category    
        single_category_params = sketch_rnn_model.get_default_hparams()
        single_category_params.data_set = [model_params.data_set[dataset_i]]
        datasets = load_dataset(data_base_dir, single_category_params)
