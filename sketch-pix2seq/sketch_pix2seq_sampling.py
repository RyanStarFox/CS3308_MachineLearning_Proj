...
def stroke5_to_custom(stroke5):
    ...
    # Filter: keep only where pen is down (p1 == 1)
    # stroke5 format: [dx, dy, p1, p2, p3]
    # p1 is at index 2
    mask = stroke5[:, 2] == 1
...
def build_category_index(png_paths):
    """Group dataloader indices by category inferred from png path."""
...
def sampling_conditional(data_dir, sampling_dir, model_dir, samples_per_category=20):
    ...
    for category, index_list in category_map.items():
        if not index_list:
            continue
        num_to_sample = min(samples_per_category, len(index_list))
        if num_to_sample == 0:
            continue
        chosen_indices = np.random.choice(index_list, size=num_to_sample, replace=False)

        for sample_idx in chosen_indices:
            png_path = test_set.png_paths[sample_idx]
            stroke = np.copy(test_set.strokes[sample_idx])
            image = test_set.load_images([png_path])
            sub_sampling_dir = os.path.join(sampling_dir, category, str(sample_idx))
            os.makedirs(sub_sampling_dir, exist_ok=True)
            print('category', category, 'rand_idx', sample_idx, 'stroke.shape', stroke.shape)
            ...
            strokes_out_3, strokes_out_5 = decode(
                ...
                z,
                temperature=0.1)
            draw_strokes(strokes_out_3, os.path.join(sub_sampling_dir, 'sample_pred_cond.svg'))
            np.savez(os.path.join(sub_sampling_dir, 'sample_pred_cond_s3.npz'), strokes=strokes_out_3)
            np.savez(os.path.join(sub_sampling_dir, 'sample_pred_cond_s5.npz'), strokes=strokes_out_5)

            custom_strokes = stroke5_to_custom(strokes_out_5)
            np.save(os.path.join(sub_sampling_dir, 'sample_pred_cond_custom.npy'), custom_strokes)

            with open(os.path.join(sub_sampling_dir, 'sample_pred_cond_tag.txt'), 'w') as f:
                f.write(category)
            ...
            for i in range(10):
                for j in range(3):
                    stroke3, stroke5 = decode(
                        sess,
                        sampling_model,
                        eval_model.hps.max_seq_len,
                        z,
                        temperature=0.1 * (i + 1))
                    stroke3_list.append([stroke3, [j, i]])
                    stroke5_list.append([stroke5, [j, i]])

            np.savez(os.path.join(sub_sampling_dir, 'sample_pred_cond_grid_s3.npz'),
                     stroke_list=np.array(stroke3_list, dtype=object))
            np.savez(os.path.join(sub_sampling_dir, 'sample_pred_cond_grid_s5.npz'),
                     stroke_list=np.array(stroke5_list, dtype=object))

            stroke_grid = make_grid_svg(stroke3_list)
...
def main(**kwargs):
    ...
    samples_per_category_ = kwargs.get('samples_per_category', 20)
    os.makedirs(sampling_dir_, exist_ok=True)

    sampling_conditional(data_dir_, sampling_dir_, model_dir_, samples_per_category_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--samples_per_category', type=int, default=20,
                        help="number of conditional samples to generate per category")
