...
def main():
    ...
    for cat in categories:
        ...
        for i, idx in enumerate(chosen):
            sample = test_arr[idx]
            # Use the original index from the dataset so the sampled data
            # keeps its original numbering.
            sample_fname_dir = os.path.join(cat_dir, str(idx))
            os.makedirs(sample_fname_dir, exist_ok=True)
            # Save the original index for clarity/traceability
            try:
                with open(os.path.join(sample_fname_dir, 'original_index.txt'), 'w', encoding='utf-8') as f:
                    f.write(str(idx))
            except Exception:
                pass
